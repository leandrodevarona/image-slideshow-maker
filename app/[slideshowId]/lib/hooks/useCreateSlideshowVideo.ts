    ...
    const createVideo = async () => {
        try {
            ...
            for (let i = 0; i < sortedSlide.length; i++) {
                await ffmpeg.writeFile(`img${i}.jpeg`, await fetchFile(sortedSlide[i].src));
            }

            const fileList = sortedSlide.map((_, i) => `file 'img${i}.jpeg'\nduration ${sortedSlide[i].duration}`).join('\n');
            ...
            for (let i = 0; i < sortedSlide.length; i++) {
                await ffmpeg.exec(['-i', `img${i}.jpeg`, '-vf', 'scale=trunc(iw/2)*2:trunc(ih/2)*2', `img${i}_scaled.jpeg`]);
            }

            const scaledFileList = sortedSlide.map((_, i) => `file 'img${i}_scaled.jpeg'\nduration ${sortedSlide[i].duration}`).join('\n');

            console.log('Scaled file list ', fileList);

            await ffmpeg.writeFile('scaled_filelist.txt', scaledFileList);

            await ffmpeg.exec(['-f', 'concat', '-safe', '0', '-i', 'scaled_filelist.txt', '-vsync', 'vfr', '-pix_fmt', 'yuv420p', 'output.mp4']);
